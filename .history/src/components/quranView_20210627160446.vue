  ...
  <div>
    <v-card rounded="lg" flat class="my-2">
      <v-card-text class="pa-10">
        <v-row>
          <div>
            <span class="teal--text">{{ surahs.verse_key }}</span>
          </div>
          <v-spacer></v-spacer>
          <div>
            <span class="black--text"> {{ surahs.text_uthmani }} </span>
          </div>
        </v-row>
        <v-row>
          <v-col class="px-0">
...
export default {
  ...
  mounted() {
    axios
      .get(" https://api.quran.com/api/v4/quran/verses/uthmani" + chapter_numberthis.id)
      .then((response) => {
