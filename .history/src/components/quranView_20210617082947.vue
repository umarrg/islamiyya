  ...
  <div>
    ...
    >
      <v-card-text class="pa-10">
        <v-row>
          <div>
            <span class="teal--text">1: {{ item.number }}</span>
          </div>
          <v-spacer></v-spacer>
          <div>
            <span> {{ item.text }} </span>
...
export default {
  ...
  mounted() {
    axios
      .get("http://api.alquran.cloud/v1/quran/quran-uthmani" + this.surahs)
      .then((response) => {
        this.surahs = response.data.data.surahs[ayahs].ayahs;
        for (var ayahs in response.data.data.surahs) {
          console.log(response.data[ayahs].name);
        }
