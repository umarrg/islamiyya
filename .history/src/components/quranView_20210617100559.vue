  ...
  <div>
    ...
    >
      <v-card-text class="pa-10">
        <v-row>
          <div>
            <span class="teal--text">1: {{ item.words.number }}</span>
          </div>
          <v-spacer></v-spacer>
          <div>
            <span class="black--text"> {{ item.words.translation.text }} </span>
          </div>
        </v-row>
        <v-row>
          <v-col class="px-0">
...
export default {
  ...
  mounted() {
    axios
      .get(" https://api.quran.com/api/v4/verses/by_chapter/2?language=en&words=true&page=1&per_page=10" + this.id)
      .then((response) => {
