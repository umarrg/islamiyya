        ...
        <v-card-text class="pa-10">
          ...
          <v-row>
            <v-col class="px-0">
              ...
              <div v-for="(item, index) in hausaTran" :key="item">
                <span class="" v-show="!show">{{ index.text  }} </span>
              </div>
              <div class="mt-5">
                <v-divider></v-divider>
                <v-divider></v-divider>
...
export default {
  ...
  methods: {
    tran(index) {
      axios
        .get(
          ` https://api.quran.com/api/v4/quran/translations/32?chapter_number=${this.id}&verse_key=${index}`
        )
        .then((res) => {
