  ...
  <div>
    ...
    >
      <v-card-text class="pa-10">
        <v-row>
          <div>
            <span class="teal--text">1:{{ item.englishName }}</span>
          </div>
          <v-spacer></v-spacer>
          <div>
            <span> <div>{{ post.categories.data.map(({name}) => name).join(' ') }}</div> </span>
          </div>
        </v-row>
        <v-row>
          <v-col class="px-0">
...
export default {
  ...
  mounted() {
    axios
      ...
      .then((response) => {
        (this.surahs = response.data.data.surahs[0].name),
          console.log(response.data.data.surahs[0].name);
      });
  },
};
</script>
