  ...
  <div>
    ...
    >
      <v-card-text class="pa-10">
        <v-row>
          <div>
            <span class="teal--text">1:{{ itemsurahs.number }}</span>
          </div>
          <v-spacer></v-spacer>
          <div>
...
export default {
  ...
  watch: {
    "$route.params.id": function(englishName) {
      this.setSurahs(englishName);
  ...
  mounted() {
    axios
      .get("http://api.alquran.cloud/v1/quran/quran-uthmani" + this.englishName)
      .then((response) => {
